...
m <- list(unit=unit,dataset=dataset,n_cells=ncol(obj),input_manifest_sha256=digest(file=file.path(inp,'input_manifest.json'),algo='sha256'),
 ...
 n_batches=length(batch_sizes),job=Sys.getenv('SLURM_JOB_ID'),source_sha256=digest(file=file.path(root,'handoff/r_reference_campaign_20260917/reference_prepare.R'),algo='sha256'),
