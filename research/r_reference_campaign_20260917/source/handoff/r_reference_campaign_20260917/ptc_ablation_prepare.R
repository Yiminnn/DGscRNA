...
# Cell order is an IO contract, not an ablation factor: keep DL splits and
# minibatch order comparable when an integration tree changes object ordering.
if(spec$group=='ALL8') {
  canonical_cells<-read.csv(file.path(root,'results/hvg_ptc_20260916_v1/ptc_paper_baseline/replay_selected_routes_full_parallel/NMT_Thyroid_Seurat_none/cells.csv'),stringsAsFactors=FALSE)$cell_id
} else canonical_cells<-rownames(read.csv(file.path(old,'prepared',group_old,'cells.csv'),row.names=1,check.names=FALSE))
stopifnot(setequal(canonical_cells,colnames(obj)))
...
if(order_changed) {
  ...
  if(spec$correction=='CCA')obj<-RunPCA(obj,features=features,npcs=30,seed.use=42,verbose=FALSE)
  if(file.exists(prep_path))stopifnot(file.rename(prep_path,paste0(prep_path,'.native_order_backup')))
}
