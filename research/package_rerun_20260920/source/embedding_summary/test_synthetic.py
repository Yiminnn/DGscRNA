...
def main():
 parser=argparse.ArgumentParser();parser.add_argument('--quantized',action='store_true');args=parser.parse_args()
 OUT=ROOT/'results/hvg_ptc_20260916_v1/package_reference_rerun_20260920/embedding_summary_validation'/('quantized_v3' if args.quantized else 'continuous_v3')
 ...
 signal=np.zeros(len(folds)) if args.quantized else np.random.default_rng(20260920).uniform(-.035,.035,size=len(folds))
    ...
    for s,space in enumerate(spec['spaces']):
     for m,method in enumerate(['KMeans','GMM','HDBSCAN_R']):
      for k in ([0] if method=='HDBSCAN_R' else spec['K']):
       value=.42+.001*i+signal[i]+.003*s+.004*m+.01*(budget=='hvg5000')+.008*(stage=='terminal090')
 ...
 inference=module('canonical_synthetic_inference',CODE/'canonical_inference.py')
 OUT.mkdir(parents=True)
 inference_proof=inference.verify(numerical,(choices,selected,patients,stats),independent,anchor,spec,OUT/'inference_verification')
 ...
 report=dict(status='passed_synthetic_only',real_campaign_aggregation_executed=False,notebook_unchanged=True,
  ...
  quantized_patient_deltas=args.quantized,canonical_inference_verification=inference_proof,
  ...
  sources={name:sha(CODE/name) for name in ['run.py','numerical.py','canonical_inference.py','test_synthetic.py','SOURCE_PROTOCOL_MANIFEST.json']})
