...
.libPaths(c(if(nzchar(reference_lib))reference_lib,.Library),include.site=FALSE)
...
for(space in cfg$space) for(method in c('SNN','HDBSCAN_R')) {
  ...
  if(!is.null(cf)) {
    ...
    if(method=='SNN') {
      obj <- FindNeighbors(obj,reduction=red,dims=seq_len(ncol(z)),k.param=as.integer(cfg$snn_k),compute.SNN=TRUE,
