...
source<-cfg$source_prepare;stopifnot(nzchar(source))
dest<-cfg$output;stopifnot(nzchar(dest))
...
if(startsWith(cfg$space,'UMAP')) {
  ...
  if(dims!=2L || cfg$embedding_seed!=42L)obj<-RunUMAP(obj,reduction='pca',dims=1:30,
    n.components=dims,n.neighbors=30L,umap.method='uwot',metric='cosine',min.dist=.3,
