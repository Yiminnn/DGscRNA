# Derived parameter-only adapter; original score_R.R density/DEG arithmetic retained.
#!/usr/bin/env Rscript
stopifnot(nzchar(Sys.getenv('SLURM_JOB_ID')))
script<-sub('^--file=','',grep('^--file=',commandArgs(),value=TRUE)[[1]])
Sys.setenv(DGSCRNA_EXECUTION_SOURCE=script)
base <- Sys.getenv('DGSCRNA_REFERENCE_OUT');stopifnot(nzchar(base))
...
cfg<-jsonlite::fromJSON(args[[3]])
all_routes<-paste(cfg$space,c('SNN','HDBSCAN_R'),sep='_')
...
marker_path <- cfg$marker_path;stopifnot(nzchar(marker_path))
...
stopifnot(identical(rownames(Embeddings(obj,'control')),colnames(obj)))
assay <- DefaultAssay(obj)
legacy_mean <- function(x) log2(Matrix::rowMeans(expm1(x))+1)
for(space in cfg$space) for(method in c('SNN','HDBSCAN_R')) {
  if(method!=cfg$method)next
  ...
  cf <- read_cache(file.path(dest,'clusters.csv'),function(p)read.csv(p,stringsAsFactors=FALSE,check.names=FALSE,colClasses='character',na.strings=NULL),
  ...
  if(!is.null(cf)) {
    ...
    if(method=='SNN') {
      ...
      obj <- FindClusters(obj,graph.name='ref_snn',resolution=cfg$resolution,algorithm=1,modularity.fxn=1,
        n.start=10,n.iter=10,random.seed=0,verbose=FALSE)
      cl <- as.character(Idents(obj))
    } else {
      fit <- dbscan::hdbscan(z,minPts=as.integer(cfg$minPts))
  ...
  m<-list(status='score_complete_DL_pending',dataset=pm$dataset,unit=unit,route=route,budget=pm$budget,seed=pm$seed,
    geometry=pm$correction,representation_control=cfg,assay=assay,n_cells=ncol(obj),n_clusters=length(ids),
