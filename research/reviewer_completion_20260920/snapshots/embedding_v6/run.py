...
def config(sample, budget, space):
    ...
    if (dest / 'config.json').exists():
        ...
        if 'conditions' in previous:
            result['conditions'] = previous['conditions']
...
def run(sample, budget, space):
    ...
    if checked(dest, 'fit_manifest.json', 'FIT_COMPLETE'):
        ...
        get_geometry(cfg)
        assert previous['config']['frozen_protocol_sha256'] == cfg['frozen_protocol_sha256']
        assert cfg['conditions'] == previous['config']['conditions']
        for condition in cfg['conditions']:
            path = Path(condition['dest'])
            assert checked(path, 'partition_manifest.json', 'PARTITION_COMPLETE')
            pm = json.loads((path / 'partition_manifest.json').read_text())
            assert pm['representation_manifest_sha256'] == sha(dest / 'representation.json')
            assert pm['clusters_sha256'] == sha(path / 'clusters.csv')
            fingerprint = json.loads((path / 'score_input_fingerprint.json').read_text())
            assert fingerprint['clusters_sha256'] == pm['clusters_sha256']
            assert fingerprint['prepare_manifest_sha256'] == sha(Path(cfg['prep']) / 'prepare_manifest.json')
            assert fingerprint['scorer_sha256'] == sha(CODE / 'score_candidates.R')
            assert checked(path, 'score_manifest.json', 'SCORE_COMPLETE')
            sm = json.loads((path / 'score_manifest.json').read_text())
            assert sm['input_fingerprint_sha256'] == sha(path / 'score_input_fingerprint.json')
            assert sm['initial_sha256'] == sha(path / 'initial_calls.csv.gz')
            td = path / 'terminal/L00_mean'
            assert checked(td, 'terminal_manifest.json', 'TERMINAL_COMPLETE')
            assert sha(td / 'terminal_manifest.json') == previous['terminal_manifest_hashes'][condition['route']]
            tm = json.loads((td / 'terminal_manifest.json').read_text())
            assert tm['score_manifest_sha256'] == sha(path / 'score_manifest.json')
