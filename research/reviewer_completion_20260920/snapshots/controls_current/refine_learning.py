...
def train_cache(x,initial,dest,provenance):
    ...
    if len(pool)==0:
        ...
        for epoch in range(PARAMS["epochs"]):
            ...
            # Direct tensor slices introduce no DataLoader RNG draw and cannot alter training.
            with torch.no_grad():
                val_n=0;val_loss=0.;val_correct=0
                for lo in range(0,len(val.indices),256):
                    idx=val.indices[lo:lo+256];vo=model(X[idx]);vy=y[idx]
                    val_loss+=float(criterion(vo,vy))*len(idx);val_n+=len(idx)
                    val_correct+=int((vo.argmax(1)==vy).sum())
            history.append(dict(epoch=epoch+1,n=total,mean_loss=weighted_loss/total,
                ...
                validation_n=val_n,validation_loss=val_loss/val_n if val_n else None,
                validation_accuracy=val_correct/val_n if val_n else None))
            if epoch+1 in [5,10,20,30]:
                cp=dest/('epoch%02d'%(epoch+1));cp.mkdir(exist_ok=True)
                with torch.no_grad():
                    probs=np.concatenate([model(torch.from_numpy(np.array(x[pool[lo:lo+256]],dtype=np.float32,copy=True))).numpy() for lo in range(0,len(pool),256)])
                conf=np.asarray([round(v,4) for v in probs.max(1)],dtype=np.float32)
                pred=classes[probs.argmax(1)];f90=initial.copy();f70=initial.copy()
                f90[pool]=np.where(conf>=.9,pred,'Unknown');f70[pool]=np.where(conf>=.7,pred,'Unknown')
                np.savez_compressed(cp/'terminal.npz',initial=initial,final090=f90,final070=f70,
                    classes=classes,pool_indices=pool,probabilities=probs,confidence_rounded=conf,
                    train_indices=train_index,validation_indices=val_index)
                torch.save(model.state_dict(),cp/'model_state.pt')
                checkpoints.append(dict(epoch=epoch+1,terminal_sha256=sha(cp/'terminal.npz'),model_sha256=sha(cp/'model_state.pt')))
    ...
    info['validation_target']='held-out marker pseudo-labels; not independent biological truth'
