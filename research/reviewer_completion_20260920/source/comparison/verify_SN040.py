...
parity=json.loads((DEST/'R_full_input_parity_SN040.json').read_text());assert parity['status']=='passed'
proof=[]
for sample in ['SN040']:
...
write_json(DEST/'verification_SN040.json',dict(status='passed',job=os.environ['SLURM_JOB_ID'],completed_at=utc(),samples=proof,
    source_sha256=sha(__file__),R_parity_sha256=sha(DEST/'R_full_input_parity_SN040.json')))
state=json.loads((DEST/'status.json').read_text());state.update(largest_pilot_verified=True,updated_at=utc())
state['jobs'].append(os.environ['SLURM_JOB_ID']);state['evidence'].extend([str(DEST/'verification_SN040.json'),str(DEST/'R_full_input_parity_SN040.json')])
