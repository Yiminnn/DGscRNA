...
from render_adaptive_figures import own_source, verify_manifest
...
def run():
    ...
    solver_rows=[]
    policy_path=CAMPAIGN/'protocol/embedding_convergence_repair_20260920_v2.json'
    policy=json.loads(policy_path.read_text())
    def display(space):return 'ICA2 adaptive' if space=='ICA2' else space
        ...
        for budget in spec['budgets']:
            for space in spec['spaces']:
                ...
                if not checked(directory, 'fit_manifest.json', 'FIT_COMPLETE') or not checked(directory / 'evaluation') or not checked(directory / 'figures'):
                    ...
                    continue
                if space=='ICA2' and not checked(directory/'figures_adaptive_v1'):
                    missing.append(f'{sample}/{budget}/{space}/adaptive_presentation')
                    continue
                cfg = json.loads((directory / 'config.json').read_text())
                rep=json.loads((directory/'representation.json').read_text())
                assert checked(directory,'representation.json','REPRESENTATION_COMPLETE')
                if space=='ICA2':
                    adaptive=verify_manifest(directory)
                    solver_rows.append(dict(sample=sample,budget=budget,space=space,space_display=display(space),
                        actual_solver=adaptive['actual_solver'],actual_iteration_cap=adaptive['actual_iteration_cap'],
                        canonical_parallel5000_status=adaptive['canonical_parallel5000_status'],
                        fallback_used=adaptive['fallback_used'],n_iter=rep['n_iter_'],
                        producer_source_sha256=rep['source_sha256'],policy_sha256=sha(policy_path),
                        representation_manifest_sha256=sha(directory/'representation.json'),
                        adaptive_figure_manifest_sha256=sha(directory/'figures_adaptive_v1/manifest.json'),
                        attempts_json=json.dumps(rep.get('attempts',[]),sort_keys=True)))
                    sources[str(directory/'figures_adaptive_v1/manifest.json')]=sha(directory/'figures_adaptive_v1/manifest.json')
                figmanifest=json.loads((directory/'figures/manifest.json').read_text())
                for name,digest in figmanifest['files'].items():assert sha(directory/'figures'/name)==digest
                sources[str(directory/'representation.json')]=sha(directory/'representation.json')
                ...
                m = m.merge(context, on='route', validate='many_to_one').assign(space=space,space_display=display(space))
                frames.append(m)
                cp = pd.read_csv(directory / 'evaluation/clustering.csv').merge(context, on='route', validate='one_to_one').assign(space=space,space_display=display(space))
                assert len(cp) == 13
                partitions.append(cp)
                sources[str(directory / 'evaluation/manifest.json')] = sha(directory / 'evaluation/manifest.json')
    summary = OUT / 'summary_adaptive_v1';summary.mkdir(exist_ok=True)
    if missing:
    ...
    assert len(solver_rows)==242
    pd.DataFrame(solver_rows).to_csv(summary/'ICA_solver_execution.csv',index=False)
    ...
    for cohort, input_rows in [('primary97', all_metrics[all_metrics.primary]), ('all121', all_metrics)]:
        ...
        for (budget, space, method), group in patients.groupby(['budget', 'space', 'method']):
            for fold in sorted(group.fold.unique()):
                ...
                choices.append(dict(cohort=cohort, budget=budget, space=space, space_display=display(space), method=method, fold=int(fold), selected_k=k,
        ...
        patient['space_display']=patient.space.map(display)
        ...
        for budget in spec['budgets']:
            ...
            for (space, method), group in current.groupby(['space', 'method']):
                ...
                comparison_rows.append(dict(cohort=cohort, budget=budget, space=space, space_display=display(space), method=method, n_patients=len(delta),
    ...
    for ax, budget in zip(axes, spec['budgets']):
        ...
        ax.axvline(0, color='#888', linewidth=.8);ax.set_yticks(y, [f'{display(space)} / {method}' for space, method in ordering], fontsize=8)
    ...
    axes[0].invert_yaxis()
    fig.suptitle('Controlled representations and clustering: patient-fold K selection\nFixed marker context; all cells retained; all contrasts shown\nICA2 adaptive: parallel caps, then deflation only after nonconvergence', fontsize=12)
    ...
    write_json(summary / 'manifest.json', dict(status='completed', expected_candidate_partitions=22022,
        ...
        adaptive_policy_sha256=sha(policy_path),adaptive_policy_name=policy['comparator_name'],
        presentation_source_manifest_sha256=sha(PRESENTATION/'SOURCE_MANIFEST.json'),
        original_summary_preserved=True,all_ICA_rows_use_adaptive_policy_label=True,
