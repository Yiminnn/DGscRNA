...
gbm_base<-'/fs/scratch/PCON0080/yimin/dgscrna/results/hvg_ptc_20260916_v1/paper_claim_validation_20260917'
gate<-fromJSON(file.path(gbm_base,'GBM_full_summary/GBM_FULL_DELIVERED.json'))
stopifnot(isTRUE(gate$PTC_compute_may_start),gate$receipt_sha256==digest(file=file.path(gbm_base,'GBM_full_summary/DELIVERY_RECEIPT.json'),algo='sha256'))
# Scientific scoring statements copied unchanged from the validated native-R campaign.
root <- '/fs/scratch/PCON0080/yimin/dgscrna'
execution_source<-sub('^--file=','',grep('^--file=',commandArgs(FALSE),value=TRUE)[[1]])
...
for(space in c('PCA30','UMAP2')) for(method in c('SNN','HDBSCAN_R')) {
  ...
  m<-list(status='score_complete_DL_pending',dataset=pm$dataset,unit=unit,route=route,
    ...
    execution_source=execution_source,source_sha256=digest(file=execution_source,algo='sha256'),
