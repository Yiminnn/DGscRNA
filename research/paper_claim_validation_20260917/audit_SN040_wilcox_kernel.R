...
for(workers in c(1L,4L))for(seed in c(1L,42L,2026L)) {
  ...
  cat('EXACT_SN040_P_VALUES',workers,seed,nrow(cases),'\n');flush.console()
}
plan(sequential)
proof<-file.path(out,'verification')
write.csv(cases,file.path(proof,'SN040_Wilcoxon_checked_pairs.csv'),row.names=FALSE)
...
writeLines(source_text,file.path(proof,'SN040_Wilcoxon_installed_source.txt'))
stopifnot(file.copy(file.path(root,'logs/claim_claim_R_pilot_7362345_2.out'),
  file.path(proof,'SN040_original_SNN_scoring_log.txt'),overwrite=TRUE))
result<-list(status='passed',job=Sys.getenv('SLURM_JOB_ID'),observed_warning_job='7362345_2',
...
write_json(result,file.path(proof,'SN040_wilcox_kernel_audit.json'),auto_unbox=TRUE,pretty=TRUE)
cat('SN040_WILCOX_KERNEL_AUDIT_PASSED',nrow(cases),length(ids),'\n')
