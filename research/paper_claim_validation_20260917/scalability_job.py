...
def run(n,repeat=0):
    ...
    n=int(n);repeat=int(repeat);sample=input_alias('DG-scRNA',n,repeat);dest=output('DG-scRNA',n,repeat);dest.mkdir(parents=True,exist_ok=True)
    if checked(dest):return
    src=OUT/'GBM'/sample/'hvg2000'
    existing=(src/'PREPARED').exists()
    assert not existing,'A resumed preparation is not an independent cold-input resource repetition'
    if repeat:os.environ['DGSCRNA_DL_CACHE_ROOT']=str(dest/'isolated_DL_cache')
    ...
    assert not tm['identical_result_reused'],'Cached DL cannot be timed as a new refinement run'
    write_json(dest/'manifest.json',dict(status='completed',n_cells=n,resource_repeat=repeat,input_sample=sample,
        pipeline_elapsed_seconds=elapsed,plotting_excluded_from_pipeline_timer=True,
        cold_preparation=not existing,DL_status=tm['dl_status'],cached_DL=tm['identical_result_reused'],
