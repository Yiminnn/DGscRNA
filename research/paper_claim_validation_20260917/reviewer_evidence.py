...
def run(phase='GBM'):
    ...
    rows=[
      ...
      ('R2-8：不同marker来源的分数分布',core,
       link(OUT/'summary/marker_context_distributions.pdf','固定原流程的16库分布')+'；'+link(OUT/'summary/marker_context_patient.csv','全部患者/阈值/阶段'),
       '主图固定HVG2000/UMAP/HDBSCAN及mean cutoff；每点为患者内样本平均。原作者标签来源重叠单独着色，不按最高分挑组织。既有PTC完整marker网格图保留。'),
      ...
      ('R2-5及R3超参数/学习曲线',status('controls_summary'),
       link(OUT/'controls_summary/MLP_all_metrics.csv','MLP控制')+'；'+link(OUT/'controls_summary/representation_all_metrics.csv','表示/聚类控制')+'；'+link(OUT/'controls_summary/MLP_training_learning_curves.pdf','实际训练曲线'),
       '原参数、种子、维数、minPts/resolution、MLP宽度/epoch；0.7/0.9共用保存概率。训练曲线及5/10/20epoch终端F1来自既有54个控制。未保存逐epoch验证loss，不虚构曲线。不存在于源码的alpha/epsilon通过方法文字纠正。参数敏感性限3个事先选定pilot。'),
