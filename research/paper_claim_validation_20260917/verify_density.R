...
unit<-readLines(file.path(base,'protocol/pilot_samples.txt'))[[as.integer(Sys.getenv('SLURM_ARRAY_TASK_ID'))+1L]]
prep<-file.path(base,'GBM',unit,'hvg2000')
...
dest<-file.path(base,'verification/pilot_density');dir.create(dest,recursive=TRUE,showWarnings=FALSE)
write_json(list(status='passed',unit=unit,job=Sys.getenv('SLURM_JOB_ID'),checks=checks,
   ...
   provenance_limit='Validates scoring on saved DEG/partitions; native labels remain unchanged through DL.'),
