...
def finish_route(source, only_arm=None, dl_prep=None):
    ...
    if dl_prep is not None:
        ...
        assert np.array_equal(pd.read_csv(dl_prep/'cells.csv').cell_id,cells.cell_id)
    ...
    refine.PARAMS['input']='Native R normalized RNA, selected genes in recorded order; not scaled expression'
    ...
    for aid in arms:
        ...
        with (cache_root/(key+'.lock')).open('a') as lock:
            ...
            tmp=dest/('predictions.csv.gz.part.'+os.environ['SLURM_JOB_ID'])
            ...
            tm=dict(status='completed',terminal_valid=True,arm=arm,family=family,
                ...
                reference_labels_used_for_fit=False,job=os.environ['SLURM_JOB_ID'],completed_at=utc())
