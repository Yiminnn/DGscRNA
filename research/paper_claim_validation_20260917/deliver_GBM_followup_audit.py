"""Publish the new archive audit as a separately verified delivery supplement."""
import json
import os
...
from common import ROOT, OUT, require_slurm, checked, sha, write_json, utc
...
def run():
    ...
    submission = OUT / 'verification/GBM_followup_content_audit_submission.json'
    source = Path(json.loads(submission.read_text())['source']) / 'verify_GBM_followup_delivery.py'
    assert sha(source) == audit['source_sha256']
    assert audit['delivery_manifest_sha256'] == sha(OUT / 'GBM_full_summary/delivery_manifest.json')
    ...
    write_json(receipt, dict(status='delivered_and_verified', files=records,
        ...
        scope='Archive-content audit supplement only; full GBM delivery has its own receipt.',
    ...
    print('GBM_FOLLOWUP_AUDIT_SUPPLEMENT_DELIVERED', flush=True)
