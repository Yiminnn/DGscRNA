...
preprocessing <- function(seurat_object){
  ...
  seurat_object.sweep <- paramSweep(seurat_object, PCs = 1:10, sct = FALSE)
  ...
  seurat_object <- doubletFinder(seurat_object, PCs = 1:10, pN = 0.25, pK = best_pK, nExp = nExp_poi, reuse.pANN = FALSE, sct = FALSE)
...
# density_score <- function(integrated_data, markers, DEG_markers_set, annotation_name, clusterings = c("seurat_clusters", "seurat.UMAP_clusters", "hdbscan_clusters", "hdbscan.UMAP_clusters", "monocle3_clusters", "monocle3.UMAP_clusters", "scCCESS_clusters"), cutoffs = c('0.5', 'mean', 'none')){
density_score <- function(integrated_data, markers, DEG_markers_set, annotation_name, clusterings = c("louvain"), cutoffs = c('0.5', 'mean', 'none')){
      ...
      for (cutoff in cutoffs){
        ...
        for (cluster_i in seq_along(levels(integrated_data)) - 1){
          ...
          for (marker_set in names(markers)){
            ...
            intersecting_genes <- base::intersect(curr_cell_type_set, top_genes) # present in the current marker
