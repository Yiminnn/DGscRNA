...
def get_stats(preds, labels, num_possible_labels):
    ...
    # PR
    all_stats["p"] = torchmetrics.Precision(
        task="multiclass", num_classes=num_possible_labels
    )(preds, labels).item()
    all_stats["r"] = torchmetrics.Recall(
        task="multiclass", num_classes=num_possible_labels
    )(preds, labels).item()
...
def run_dgscrna(folder_loc, data_name, study_sets, EPOCHS=10):
    ...
    if data_name.endswith(".h5ad"):
        data_w_batch_corr_annotated = scanpy.read_h5ad(data_w_batch_corr_annotated_loc)
    elif data_name.endswith(".loom"):
        data_w_batch_corr_annotated = scanpy.read_loom(
            data_w_batch_corr_annotated_loc, sparse=False
        )
    print(data_name)
    ...
    analyze_columns = [
        i
        for i in orig_annotations.columns
        if ("_none" in i) or ("_0.5" in i) or ("_mean" in i)
    ]  # ('clusters' in i) and

    orig_annotations.index = [i.split("_")[0] for i in orig_annotations.index]
    orig_annotations.index = (
        orig_annotations.index.values
        + "_"
        + np.arange(start=0, stop=orig_annotations.shape[0]).astype(str)
    )
    ...
    if os.path.isfile(annotation_file):
        final_res = pd.read_csv(annotation_file, index_col=0, low_memory=False)
    else:
    ...
    for different_annotation in analyze_columns:
        ...
        if (final_cell_types == "Undecided").sum() == final_cell_types.shape[0]:
            ...
            final_res[different_annotation + "_DGscRNA"] = [
                "Undecided"
            ] * final_cell_types.shape[0]
            final_res.to_csv(annotation_file)
            continue
        ...
        agreed_labels = [
            bag_of_classes[i]
            for i in orig_annotations.loc[agreed_cells, different_annotation]
        ]
        non_agreed_labels = [
            bag_of_classes[i]
            for i in orig_annotations.loc[disagreed_cells, different_annotation]
        ]

        agreed_cells_idx = orig_annotations.loc[
            orig_annotations[different_annotation] != "Undecided", "cell_id"
        ]
        disagreed_cells_idx = orig_annotations.loc[
            orig_annotations[different_annotation] == "Undecided", "cell_id"
        ]
        ...
        X_train = torch.tensor(
            data_w_batch_corr_annotated.X[agreed_cells_idx,], dtype=torch.float
        )

        y_test = torch.tensor(non_agreed_labels, dtype=torch.long)
        X_test = torch.tensor(
            data_w_batch_corr_annotated.X[disagreed_cells_idx,], dtype=torch.float
        )
        ...
        params = {"batch_size": 256, "shuffle": False, "num_workers": 8}
        ...
        model = DeepModel(
            input_shape=data_w_batch_corr_annotated.X.shape[1],
            class_shape=len(bag_of_classes),
        )
        ...
        for epoch in range(EPOCHS):  # loop over the dataset multiple times
            ...
            all_stats = get_stats(
                all_preds, all_labels, num_possible_labels=len(bag_of_classes)
            )
        ...
        class_correct = list(0.0 for i in range(len(bag_of_classes)))
        class_total = list(0.0 for i in range(len(bag_of_classes)))
        ...
        all_val_stats = get_stats(
            all_val_preds, all_val_labels, num_possible_labels=len(bag_of_classes)
        )
        ...
        print(
            [
                i
                for i in bag_of_classes
                if bag_of_classes[i] not in torch.unique(all_val_preds).tolist()
            ]
        )
        ...
        all_test_stats = get_stats(
            all_test_preds, all_test_labels, num_possible_labels=len(bag_of_classes)
        )
        ...
        final_cell_types.iloc[disagreed_cells_idx.iloc[correct_index]] = [
            rev_bag_of_classes[i] for i in all_test_preds[correct_index].numpy()
        ]
            ...
            for ct in final_cell_types:
                ...
                if "HPA" in str(ct):  # == 'HPA':
                    ...
                    if (
                        entry[0] == "Undecided"
                        or entry[0] == "Unknown"
                        or entry[0] == "No_Annotation"
                    ):
                        tissue_general.append(entry[0])
                    elif entry[0] == "0":
                        tissue_general.append("No_Annotation")
                    elif (entry[0] == "CellMarker_normal") or (
                        entry[0] == "CellMarker_cancer"
                    ):
