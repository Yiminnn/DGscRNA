...
plan(multicore,workers=4L);options(future.globals.maxSize=16*1024^3)
...
dest <- file.path(base,'ptc_paper_baseline/replay_selected_routes_full_parallel',route)
...
deg_file <- file.path(dest,'DEG_full_integrated2000.rds')
if(file.exists(deg_file)) {
  deg <- readRDS(deg_file)
} else {
  cat(format(Sys.time()),'Full-assay FindAllMarkers',clname,'clusters',length(ids),'\n');flush.console()
...
con <- gzfile(file.path(dest,'DEG_full_integrated2000.csv.gz'),'wt');write.csv(deg,con,row.names=FALSE);close(con)
...
manifest <- list(status='scoring_complete_DL_pending',route=route,library=lib,cutoff=cutoff,cluster_column=clname,
  ...
  sources=list(checkpoint=checkpoint,checkpoint_sha256=digest(file=checkpoint,algo='sha256'),
    ...
    script_sha256=digest(file=file.path(root,'handoff/ptc_paper_baseline_20260916/replay_selected_routes_full_parallel.R'),algo='sha256')),
  gene_test_execution='Four forked workers within a four-CPU allocation; same full-gene tests and statistics',compatibility='Seurat 5 wilcox_limma and explicit v4 mean/logFC/default thresholds; original density_score executed unmodified',
  scoring_features=2000L,n_cells=length(cells),n_clusters=length(ids),n_initial_undecided=sum(initial=='Undecided'),
