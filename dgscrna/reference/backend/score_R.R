...
allocated_workers<-as.integer(Sys.getenv('SLURM_CPUS_PER_TASK','4'))
requested_workers<-Sys.getenv('DGSCRNA_DEG_WORKERS','')
stopifnot(!is.na(allocated_workers),allocated_workers>=1L)
if(nzchar(requested_workers)) {
  stopifnot(grepl('^[1-9][0-9]*$',requested_workers))
  requested_workers_count<-as.integer(requested_workers)
  stopifnot(!is.na(requested_workers_count),requested_workers_count>=1L)
} else requested_workers_count<-4L
deg_workers<-min(4L,allocated_workers,requested_workers_count)
...
# Honor the explicit limit for every assay. Retain the historical two-worker
# default for large assays when no override is supplied; same DEG statistics.
if(pm$features$scoring>10000L && !nzchar(requested_workers))
  deg_workers<-min(deg_workers,2L)
...
cells <- read.csv(file.path(prep,'cells.csv'),stringsAsFactors=FALSE,check.names=FALSE,
  colClasses='character',na.strings=NULL)
stopifnot(identical(colnames(obj),cells$cell_id))
publish <- function(path,writer) {
  tmp<-paste0(path,'.part.',execution_id(),'.',Sys.getpid())
  writer(tmp)
...
read_cache <- function(path,reader,valid) {
  ...
  tryCatch({x<-reader(path);stopifnot(isTRUE(valid(x)));x},error=function(e) {
    ...
    event<-list(path=path,preserved_as=preserved,error=conditionMessage(e),job=execution_id())
    cat(toJSON(event,auto_unbox=TRUE),'\n',file=file.path(prep,paste0('cache_recovery_',execution_id(),'.jsonl')),append=TRUE)
...
if(!is.null(embedding)) {
  ...
  obj <- RunUMAP(obj,reduction='pca',dims=1:30,n.components=2,n.neighbors=30L,
       umap.method='uwot',metric='cosine',min.dist=0.3,seed.use=pm$seed,verbose=FALSE)
...
for(space in c('PCA30','UMAP2')) for(method in c('SNN','HDBSCAN_R')) {
  ...
  if(file.exists(file.path(dest,'SCORE_COMPLETE'))) {
    ...
    prior<-fromJSON(file.path(dest,'score_manifest.json'))
    stopifnot(prior$marker_source_sha256==digest(file=marker_path,algo='sha256'),
              prior$source_expression_sha256==pm$expression_sha256,prior$seed==pm$seed)
    next
  }
  started <- Sys.time()
  red <- if(space=='PCA30')'pca' else 'umap'
  z <- Embeddings(obj,red)
  cf <- read_cache(file.path(dest,'clusters.csv'),function(p)read.csv(p,stringsAsFactors=FALSE,check.names=FALSE,
      colClasses='character',na.strings=NULL),
  ...
  m<-list(status='score_complete_DL_pending',dataset=pm$dataset,unit=unit,route=route,budget=pm$budget,seed=pm$seed,
    ...
    source_expression_sha256=pm$expression_sha256,
    ...
    reference_labels_used_for_fit=FALSE,DEG_workers=deg_workers,job=execution_id(),
    runtime_R_library_paths=.libPaths(),
    runtime_R_package_paths=setNames(lapply(c('Seurat','Matrix','jsonlite','digest','future','dbscan','limma'),function(p)normalizePath(find.package(p))),
                                    c('Seurat','Matrix','jsonlite','digest','future','dbscan','limma')),
