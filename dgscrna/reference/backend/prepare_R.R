...
if(file.exists(file.path(dest,'PREPARED'))) {
  ...
  prior<-fromJSON(file.path(dest,'prepare_manifest.json'))
  stopifnot(prior$input_manifest_sha256==digest(file=file.path(inp,'input_manifest.json'),algo='sha256'),
            prior$budget==budget,prior$seed==seed)
  quit(status=0)
}
started<-Sys.time();set.seed(seed)
log<-function(...) {cat(format(Sys.time()),..., '\n');flush.console()}
publish<-function(path,writer) {tmp<-paste0(path,'.part.',execution_id(),'.',Sys.getpid());writer(tmp);stopifnot(file.rename(tmp,path))}
...
genes<-read.csv(file.path(inp,'genes.csv'),stringsAsFactors=FALSE,check.names=FALSE,
  colClasses='character',na.strings=NULL)$gene
...
m<-list(status='completed',unit=sample,dataset=if(is.null(im$dataset)) 'user_supplied_single_sample' else im$dataset,sample=sample,condition=condition,budget=budget,
  ...
  job=execution_id(),elapsed_seconds=as.numeric(difftime(Sys.time(),started,units='secs')))
